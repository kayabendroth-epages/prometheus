...
func (cd *ConsulDiscovery) Sources() []string {
	...
	clientConf.HttpClient = utility.NewDeadlineClient(5 * time.Second)
	...
	for name := range srvs {
		// The consul service is always present but does not export metrics, drop it.
		// Other services not exporting for Prometheus have to be filtered manually
		// via relabeling.
		if name == "consul" {
			continue
		}
